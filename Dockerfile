...
# 기존 가상환경 복사 (venv_py311 디렉토리를 사용)
COPY venv_py311 /opt/conda/envs/tts

# Flask 및 필요한 패키지 설치 (가상환경이 제대로 작동하지 않는 경우를 대비)
RUN pip install --no-cache-dir \
    flask==2.2.5 \
    pydub==0.25.1 \
    soundfile==0.12.1 \
...
# app 패키지 전체를 복사
COPY config/ config/  
# config 패키지도 필요 (Config 클래스가 여기 있음)
...
# 애플리케이션 실행
CMD ["conda", "run", "--no-capture-output", "-n", "tts", "python", "run.py"]